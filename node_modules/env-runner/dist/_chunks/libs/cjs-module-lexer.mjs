...
function parse(E, g = "@") {
	...
	const w = E.length + 1, D = (A.__heap_base.value || A.__heap_base) + 4 * w - A.memory.buffer.byteLength;
	D > 0 && A.memory.grow(Math.ceil(D / 65536));
	const G = A.sa(w);
	(B ? C : I)(E, new Uint16Array(A.memory.buffer, G, w));
	const S = A.parseCJS(G, E.length, 0, 0, 0);
	if (S) {
		const B = /* @__PURE__ */ new Error(`Parse error ${g}${A.e()}:${E.slice(0, A.e()).split("\n").length}:${A.e() - E.lastIndexOf("\n", A.e() - 1)}`);
		throw Object.assign(B, { idx: A.e() }), 5 !== S && 6 !== S && 7 !== S || Object.assign(B, { code: "ERR_LEXER_ESM_SYNTAX" }), B;
	}
	let o = /* @__PURE__ */ new Set(), R = /* @__PURE__ */ new Set(), y = /* @__PURE__ */ new Set();
	for (; A.rre();) {
		const B = Q(E.slice(A.res(), A.ree()));
		B && R.add(B);
	}
	for (; A.ru();) y.add(Q(E.slice(A.us(), A.ue())));
	for (; A.re();) {
		let B = Q(E.slice(A.es(), A.ee()));
...
function Q(A) {
	...
	try {
		const B = function(A) {
			...
			else if ("'" === B && A.length > 1 && "'" === A[A.length - 1] && -1 === A.indexOf("\"")) try {
...
function E(A, B) {
	...
	switch (++B.v, Q) {
		...
		case "f": return "\f";
		case "v": return "\v";
		case "x": return function(A, B) {
			const Q = g(A[B.v]);
			++B.v;
			const E = g(A[B.v]);
			return ++B.v, String.fromCodePoint(16 * Q + E);
		}(A, B);
		case "u": return function(A, B) {
			let Q = 0;
			if ("{" === A[B.v]) {
				++B.v;
				do {
